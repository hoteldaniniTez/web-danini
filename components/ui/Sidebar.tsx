...
import { usePathname } from 'next/navigation'; // Importamos usePathname para detectar cambio de ruta
...
export const Sidebar = () => {
    ...
    // Hook para detectar en qué url estamos
    const pathname = usePathname();

    // Lógica Experta: Detectar navegación
    useEffect(() => {
        // 1. Si el menú está abierto y cambiamos de ruta, cerrarlo.
        if (isSideMenuOpen) {
            closeMenu();
        }
        // 2. Scroll al inicio absoluto de la página
        window.scrollTo(0, 0);
    ...
    return (
        <div>
            ...
            <div
                className={clsx(
                    "fixed inset-0 z-50 bg-black/70 backdrop-blur-[2px] transition-all duration-700",
                    {
            ...
            <nav
                className={clsx(
                    // CAMBIO CLAVE: h-[100dvh] en lugar de h-screen para arreglar el corte en móviles
                    "fixed top-0 right-0 z-50 w-[85%] sm:w-[450px] h-dvh bg-linear-to-b from-[#003629] to-[#00281f] backdrop-blur-xl border-l border-r border-[#d79623]/30 text-[#f0e1ce] shadow-[0_0_50px_rgba(0,0,0,0.5)] transform transition-transform duration-700 cubic-bezier(0.25, 1, 0.5, 1) flex flex-col overflow-hidden",
                    {
            ...
            >
                ...
                {/* Header */}
                <div className="flex justify-between items-end p-8 pt-10 relative z-10 shrink-0">
                    <div>
                ...
                {/* Lista Navegación */}
                {/* TRUCO EXPERTO: key={isSideMenuOpen...} fuerza a React a "reiniciar" este div 
                    cuando el menú se cierra/abre. Esto cierra automáticamente los acordeones (habitaciones) */}
                <div
                    key={isSideMenuOpen ? 'open' : 'closed'}
                    className="flex-1 overflow-y-auto py-6 px-6 space-y-2 relative z-10 scrollbar-hide"
                >
                ...
                {/* Footer */}
                {/* shrink-0 evita que el flexbox aplaste el footer si la pantalla es muy pequeña */}
                <div className="p-8 bg-[#00221a]/90 border-t border-[#d79623]/20 relative z-10 shrink-0 pb-safe">
                    <div className="flex flex-col gap-6">
                        <div className="flex justify-between px-4">
                            <a href="https://www.facebook.com/HotelDaniniTeziutlan" target="_blank" rel="noopener noreferrer" className="text-[#f0e1ce]/60 hover:text-[#d79623] hover:scale-125 transition-all duration-300"><Facebook size={26} strokeWidth={1.5} /></a>
                            <a href="https://www.instagram.com/hoteldanini_teziutlan" target="_blank" rel="noopener noreferrer" className="text-[#f0e1ce]/60 hover:text-[#d79623] hover:scale-125 transition-all duration-300"><Instagram size={26} strokeWidth={1.5} /></a>
                            <a href="https://goo.gl/maps/mixF55zxoMPCGuhE8" target="_blank" rel="noopener noreferrer" className="text-[#f0e1ce]/60 hover:text-[#d79623] hover:scale-125 transition-all duration-300"><MapPin size={26} strokeWidth={1.5} /></a>
